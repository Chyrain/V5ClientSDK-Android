...
import com.v5kf.client.lib.utils.ImageLoader;
import com.v5kf.client.lib.utils.MediaLoader;
import com.v5kf.client.lib.utils.URLCache;
...
//			Logger.w(TAG, "device_token not set!");
	...
	private void getPictureService(String url, String auth, final V5Message message, final MessageSendCallback handler) {
		HttpUtil.getPicService(url, auth, new HttpResponseHandler(mContext) {
...
//							config.setWxytauth(authorization);
	...
	private void postImageAndSend(final V5ImageMessage imageMessage, String url, String authorization, 
		...
		HttpUtil.postLocalImage(url, filePath, authorization, null, new HttpResponseHandler(mContext) {
...
//						String url = data.optString("url");
	...
	protected void postMediaAndSend(final V5Message message, String url,
		...
		Logger.i(TAG, "[postMediaAndSend] filePath:" + filePath);
		final File file = new File(filePath);
		HttpUtil.postLocalMedia(message, filePath, url, authorization, magicContext, new HttpResponseHandler(mContext) {
...
//						String url = data.optString("url");
						...
						if (!TextUtils.isEmpty(access_url)) {
							if (message.getMessage_type() == V5MessageDefine.MSG_TYPE_VOICE) {
								...
								// 删除临时语音文件，重命名
								String cachePath = MediaLoader.copyPathToFileCche(getContext(), file, access_url);
			...
			public void onFailure(int statusCode, String responseString) {
				...
				if (statusCode == -1) {
					sendFailedHandle(
							handler, 
							message, 
							V5ExceptionStatus.ExceptionNoAudioPermission, 
							responseString);
				} else {
					sendFailedHandle(
							handler, 
...
//							}
	...
	public void onAppGoForeGround() {
		...
		if (V5ClientService.isConnected()) {
			...
			// [修改]点击通知 重新获取消息，改成手动获取消息[在对话界面的onStart时发送]
			/* onStart -> 刷新最新会话数据 */
			//this.updateMessages();
		} else {
			V5ClientService.reConnect(mContext);
...
//			mContext.startService(i);
	...
	public void onAppGoBackground() {
		Logger.d(TAG, "[onAppGoBackground]");
	...
	public void sendMessage(V5Message message, MessageSendCallback handler) {
		...
		if (message.getMessage_type() == V5MessageDefine.MSG_TYPE_IMAGE) {
			...
			if (voiceMessage.getUrl() == null && voiceMessage.getFilePath() != null) {
				V5ClientConfig config = V5ClientConfig.getInstance(mContext);
				if (null != config && null != config.getAuthorization()) {
					String url = V5ClientConfig.getMediaAuthURL() + config.getAuthorization();
...
//		LocalBroadcastManager.getInstance(mContext).sendBroadcast(sendIntent);
	...
	public void updateMessages() {
		this.getCurrentMessages(0, 0);
		this.getStatus();
	}
	
	/**
	 * 获取会话状态 status = 0-机器人服务  1-排队等待 2-人工客服
	 ...
	 * @param callback
	 */
	public void getCurrentMessages(int offset, int size) {
...
//	}
	...
	protected static class OnMessageRunnable implements Runnable {
		...
		@Override
		public void run() {
			Logger.d(TAG, "onMessage post->run msgBean:" + msgBean + " messageListener:" + V5ClientAgent.getInstance().getMessageListener());
