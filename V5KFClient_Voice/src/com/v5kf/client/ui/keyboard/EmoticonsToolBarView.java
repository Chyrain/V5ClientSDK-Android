...
public class EmoticonsToolBarView extends RelativeLayout implements IEmoticonsKeyboard {
    ...
    public EmoticonsToolBarView(Context context, AttributeSet attrs) {
        ...
        inflater.inflate(UIUtil.getIdByName(context, "layout", "v5_view_emoticonstoolbar"), this);
        this.mContext = context;
        findView();
    }

    private void findView() {
        hsv_toolbar = (HorizontalScrollView) findViewById(UIUtil.getIdByName(getContext(), "id", "hsv_toolbar"));
        ly_tool = (LinearLayout) findViewById(UIUtil.getIdByName(getContext(), "id", "ly_tool"));
    }
...
//                    int childX = (int)ViewHelper.getX(ly_tool.getChildAt(position));
                    int childX = (int)(ly_tool.getChildAt(position).getLeft()); // getX()
    ...
    public void setToolBtnSelect(int select) {
        ...
        for (int i = 0; i < mToolBtnList.size(); i++) {
            if (select == i) {
                mToolBtnList.get(i).setBackgroundColor(getResources().getColor(UIUtil.getIdByName(getContext(), "color", "v5_toolbar_btn_select")));
            } else {
                mToolBtnList.get(i).setBackgroundColor(getResources().getColor(UIUtil.getIdByName(getContext(), "color", "v5_toolbar_btn_nomal")));
            }
        }
    }
    ...
    public void addData(int rec){
        if(ly_tool != null){
            ...
            View toolBtnView = inflater.inflate(UIUtil.getIdByName(getContext(), "layout", "v5_item_toolbtn"), null);
            ImageView iv_icon = (ImageView)toolBtnView.findViewById(UIUtil.getIdByName(getContext(), "id", "iv_icon"));
    ...
    public void setBuilder(EmoticonsKeyboardBuilder builder) {
        ...
        for(EmoticonSetBean bean : mEmoticonSetBeanList){
            LayoutInflater inflater = (LayoutInflater)mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            View toolBtnView = inflater.inflate(UIUtil.getIdByName(getContext(), "layout", "v5_item_toolbtn"),null);
//            View v_spit = (View)toolBtnView.findViewById(UIUtil.getIdByName(getContext(), "id", "v_spit"));
            ImageView iv_icon = (ImageView)toolBtnView.findViewById(UIUtil.getIdByName(getContext(), "id", "iv_icon"));
            LinearLayout.LayoutParams imgParams = new LinearLayout.LayoutParams(Utils.dip2px(mContext,mBtnWidth), LayoutParams.MATCH_PARENT);
            iv_icon.setLayoutParams(imgParams);
            ly_tool.addView(toolBtnView);
