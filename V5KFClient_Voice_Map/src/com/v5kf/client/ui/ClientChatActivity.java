...
//			}
	...
	private void findView() {
		...
		mRightBtn = (Button) findViewById(UIUtil.getIdByName(this, "id", "header_right_btn"));
		mChatListView = (ListView) findViewById(UIUtil.getIdByName(this, "id", "id_recycler_msgs"));
		mRefreshLayout = (SwipeRefreshLayout) findViewById(UIUtil.getIdByName(this, "id", "id_swipe_refresh"));
		mKeyBar = (EmoticonsKeyBoardBar) findViewById(UIUtil.getIdByName(this, "id", "chat_activity_keybar"));
		
		mInputEt = mKeyBar.getEt_chat();
		
		/* 语音有关 */
		mBtnVoice = mKeyBar.getBtn_voice(); // 长按语音输入按钮
        layout_record = (RelativeLayout) findViewById(UIUtil.getIdByName(this, "id", "id_mask_view"));
		tv_voice_tips = (TextView) findViewById(UIUtil.getIdByName(this, "id", "tv_voice_tips"));
		tv_voice_title = (TextView) findViewById(UIUtil.getIdByName(this, "id", "tv_voice_title"));
...
//		mRightBtn.setBackgroundResource(UIUtil.getIdByName(this, "drawable", "v5_action_bar_quit"));
...
//				dialog.setContent(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_on_exit_service"));
//				dialog.setContentViewGravity(Gravity.CENTER);
//				dialog.setOnClickListener(new WarningDialogListener() {					
//					@Override
//					public void onClick(View view) {
//						if (view.getId() == UIUtil.getIdByName(ClientChatActivity.this, "id", "btn_dialog_warning_right")) {
...
//		});
	...
	private void initAppfunc() {
		View viewApps =  mInflater.inflate(UIUtil.getIdByName(this, "layout", "v5_view_apps"), null);
		...
		pageApps.setFuncItemClickListener(new FuncItemClickListener() {			
			...
			public void onFuncItemClick(View v, AppBean bean) {
				if (!isConnected) {
					showToast(UIUtil.getIdByName(getApplicationContext(), "string", "v5_waiting_for_connection"));
					return;
				}				
				switch (bean.getIcon()) {
				...
				case "v5_icon_relative_ques": // 相关问题					
					mQuestionDesc.setText(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_relative_question"));
					showQuestionList();
					break;
					
				case "v5_icon_photo": // 图片
					if (UIUtil.hasPermission(getApplicationContext(), "android.permission.WRITE_EXTERNAL_STORAGE")) {
						openSystemPhoto();
					} else {
						showWarningDialog(UIUtil.getIdByName(getApplicationContext(), "string", "v5_permission_photo_deny"), null);
					}
					break;
				case "v5_icon_camera": // 拍照
					if (UIUtil.hasPermission(getApplicationContext(), "android.permission.CAMERA") && 
							UIUtil.hasPermission(getApplicationContext(), "android.permission.WRITE_EXTERNAL_STORAGE")) {
						cameraPhoto();
					} else {
						showWarningDialog(UIUtil.getIdByName(getApplicationContext(), "string", "v5_permission_camera_deny"), null);
					}
					break;
...
//					showToast(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_artificial_service"));
	...
	private void initQuestionList() {
		View viewQues =  mInflater.inflate(UIUtil.getIdByName(this, "layout", "v5_view_robot_candidate"), null);
		mQuestionDesc = (TextView) viewQues.findViewById(UIUtil.getIdByName(this, "id", "id_candidate_desc"));
		mQuestionDesc.setVisibility(View.VISIBLE);
        mKeyBar.add(viewQues); // 位置2-添加常见问答、相关问题显示界面
        
        /* 机器人候选答案列表初始化 */
        mQuesList = (ListView) viewQues.findViewById(UIUtil.getIdByName(this, "id", "id_candidate_list"));
        /* 适配器初始化 */
...
//                    		mChatListView.smoothScrollToPosition(mDatas.size() - 1);
	...
	private void initChatListView() {
        ...
        mRefreshLayout.setColorSchemeColors(
        		UIUtil.getIdByName(this, "color", "v5_green"), 
        		UIUtil.getIdByName(this, "color", "v5_red"), 
        		UIUtil.getIdByName(this, "color", "v5_blue"),
        		UIUtil.getIdByName(this, "color", "v5_yellow"));
	}
	...
	protected void onSendClick(String msg) {
		if (TextUtils.isEmpty(mInputEt.getText())) {
			showToast(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_msg_input_empty"));
			return;
		}
							...
							try {
								...
								if (resp.get("state").equals("ok") && resp.getInt("total") > 0) {
									...
									for (int i = 0; i < arr.length(); i++) {
										...
										V5TextMessage msg = V5MessageManager.getInstance().obtainTextMessage(str);
										mQuesContents.add(msg);
									}						
									mQuesAdapter.notifyDataSetChanged();
									mQuestionDesc.setText(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_hot_question"));
									showQuestionList();
								} else {
									showToast(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_toast_hot_reqs_empty"));
								}
...
//				dialog.setContent(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_connect_error"));
//				dialog.setContentViewGravity(Gravity.CENTER);
//				dialog.setOnClickListener(new WarningDialogListener() {					
//					@Override
//					public void onClick(View view) {
...
//				showWarningDialog(UIUtil.getIdByName(getApplicationContext(), "string", "v5_error_record_not_permit"), null);
//				break;
			default:
				break;
			}
...
//		}
	...
	public void onChatItemClick(View v, int position, int viewType) {
		...
		if (v.getId() == UIUtil.getIdByName(ClientChatActivity.this, "id", "id_news_layout")) {
		...
		} else if (v.getId() == UIUtil.getIdByName(ClientChatActivity.this, "id", "id_left_location_layout")
				|| v.getId() == UIUtil.getIdByName(ClientChatActivity.this, "id", "id_right_location_layout")) {
...
//				}  
		...
		} else if (v.getId() == UIUtil.getIdByName(ClientChatActivity.this, "id", "id_msg_fail_iv")) { // 重发
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQ_CODE_LOCATION) {
			switch (resultCode) {
			case LocationMapActivity.RESULT_CODE_GET_LOCATION:
				if (data == null) {
					showToast(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_on_location_empty"));
				} else {
	...
	public void onRefresh() {
		...
		//if (isCurrentFinish) {
			if (isHistoricalFinish) {
				showToast(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_no_more_messages"));
				if (mRefreshLayout.isRefreshing()) {
					mRefreshLayout.setRefreshing(false);
				}
				return;
...
//        startActivityForResult(intent, REQUEST_CODE_PHOTO);
    ...
    private void cameraPhoto() {
        ...
        if (!sdStatus.equals(Environment.MEDIA_MOUNTED)) {
            showToast(UIUtil.getIdByName(this, "string", "v5_no_sdcard"));
            return;
        }
	...
	class VoiceTouchListen implements View.OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				if (!UIUtil.hasPermission(getApplicationContext(), "android.permission.RECORD_AUDIO")) {
					showWarningDialog(UIUtil.getIdByName(getApplicationContext(), "string", "v5_error_record_not_permit"), null);
					return false;
				}
				v.setPressed(true);
			...
			case MotionEvent.ACTION_MOVE: {
				if (event.getY() < 0) {
					tv_voice_tips.setText(getString(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_chat_voice_cancel_tips")));
					//tv_voice_tips.setTextColor(Color.RED);
				} else {
					tv_voice_tips.setText(getString(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_chat_voice_up_tips")));
	...
	private void showVoiceRecordingProgress() {
		...
		tv_voice_tips.setText(getString(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_chat_voice_cancel_tips")));
		// 录音动画
		Animation operatingAnim = AnimationUtils.loadAnimation(this, UIUtil.getIdByName(ClientChatActivity.this, "anim", "v5_round_loading"));
...
//		layout_record.setVisibility(View.GONE);
	...
	public void onErrorOfSpeech(int errorCode, String desc) {
		...
		case VoiceErrorCode.E_RECORD_NOT_PERMIT: // 可能未取得录音权限
			showWarningDialog(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_error_record_not_permit"), null);
			break;
		case VoiceErrorCode.E_NOSDCARD: // 存储路径错误
			showWarningDialog(UIUtil.getIdByName(ClientChatActivity.this, "string", "v5_error_no_sdcard"), null);
			break;
		}
	}
