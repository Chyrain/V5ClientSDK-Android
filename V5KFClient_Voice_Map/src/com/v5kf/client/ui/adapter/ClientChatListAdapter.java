...
public class ClientChatListAdapter extends BaseAdapter {
	...
	public ClientChatListAdapter(Activity context, List<V5ChatBean> messages, OnChatListClickListener listener, boolean avatar) {
		super();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
        	switch (viewType) {
            case TYPE_LEFT_TEXT:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_from_msg"), parent, false) ;
            	holder = new ViewHolder(viewType, convertView);
            	break;
            	
            case TYPE_RIGHT_TEXT:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_to_msg"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;
            	
            case TYPE_SINGLE_NEWS:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_single_news"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;
            	
            case TYPE_NEWS:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_multi_news"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;
            	
            case TYPE_LOCATION_R:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_to_location"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;

            case TYPE_LOCATION_L:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_from_location"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;
            	
            case TYPE_IMG_L:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_from_img"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;

            case TYPE_IMG_R:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_to_img"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;
            	
            case TYPE_VOICE_L:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_from_voice"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;

            case TYPE_VOICE_R:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_to_voice"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;
            	
            case TYPE_TIPS:
            	convertView = mInflater.inflate(UIUtil.getIdByName(mContext, "layout", "v5_item_chat_tips"), parent, false);
            	holder = new ViewHolder(viewType, convertView);
            	break;
        ...
        case TYPE_SINGLE_NEWS: { // 单图文
			...
			ImageLoader imgLoader = new ImageLoader(mContext, true, UIUtil.getIdByName(mContext, "drawable", "v5_img_src_loading"));
        	imgLoader.DisplayImage(article.getPic_url(), holder.mNewsPic);
        	
        	convertView.findViewById(UIUtil.getIdByName(mContext, "id", "id_news_layout")).setOnClickListener(new ItemClick(position, viewType, holder));
		}
		break;
		...
		case TYPE_LOCATION_R: { // 位置-发出
        	...
        	holder.mLbsDescTv.setText(mContext.getString(UIUtil.getIdByName(mContext, "string", "v5_loading")));
        	UIUtil.getLocationTitle(mContext, lat, lng, holder.mLbsDescTv, null);
        	
        	convertView.findViewById(UIUtil.getIdByName(mContext, "id", "id_right_location_layout"))
		...
		case TYPE_LOCATION_L: { // 位置-接收
        	...
        	convertView.findViewById(UIUtil.getIdByName(mContext, "id", "id_left_location_layout"))
					.setOnClickListener(new ItemClick(position, viewType, holder));
		}
		break;
...
//			Logger.d(TAG, position + " _R list load Voice ----- duration:" + voiceMessage.getDuration() + " filePath:" + voiceMessage.getFilePath());
			...
			if (voiceMessage.getFilePath() != null && voiceMessage.getDuration() > 0) { // 已加载则不需要loadMedia
//				Logger.d(TAG, "---已加载---");
				break;
			} else {
//				Logger.d(TAG, "---首次加载---");
...
//        	Logger.d(TAG, "_R Voice url:" + url + " sendState:" + voiceMessage.getState());
        	MediaLoader mediaLoader = new MediaLoader(mContext, holder, new MediaLoader.MediaLoaderListener() {
...
//					Logger.w(TAG, "_R MediaLoader Voice onSuccess:" + media.getDuration() + media.getLocalPath());
		...
		case TYPE_VOICE_L: { // 语音-接收
			convertView.findViewById(UIUtil.getIdByName(mContext, "id", "id_left_voice_layout"))
			.setOnClickListener(new ItemClick(position, viewType, holder));
...
//					Logger.w(TAG, "_L MediaLoader Voice onSuccess:" + media.getDuration() + media.getLocalPath());
		...
		case TYPE_TIPS:
			holder.mMsg.setText(message.getDefaultContent(mContext));
			break;
		...
		default: {
			...
			if (message.getDirection() == V5MessageDefine.MSG_DIR_TO_WORKER) {
				holder.mMsg.setBackgroundResource(UIUtil.getIdByName(mContext, "drawable", "v5_list_to_textview_bg"));
			} else if (message.getDirection() == V5MessageDefine.MSG_DIR_FROM_ROBOT ||
					message.getDirection() == V5MessageDefine.MSG_DIR_COMMENT) {
				holder.mMsg.setBackgroundResource(UIUtil.getIdByName(mContext, "drawable", "v5_list_from_robot_bg"));
			} else if (message.getDirection() == V5MessageDefine.MSG_DIR_TO_CUSTOMER) {
				holder.mMsg.setBackgroundResource(UIUtil.getIdByName(mContext, "drawable", "v5_list_from_textview_bg"));
			}
        ...
        if (holder.mAvatar != null && !showAvatar) { // 不显示头像
        	...
        	if (message.getDirection() == V5MessageDefine.MSG_DIR_TO_WORKER) {
        		photoDefaultId = UIUtil.getIdByName(mContext, "drawable", "v5_avatar_customer");
        	...
        	ImageLoader mapImgLoader = new ImageLoader(mContext, true, photoDefaultId);
...
//    }
	...
	class ViewHolder {
        ...
        public ViewHolder(int viewType, View itemView) {
            switch (viewType) {
            case TYPE_LEFT_TEXT:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_date"));
            	mMsg = (EmojiconTextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_text"));
            	mAvatar = (RoundImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_avatar"));
            	break;
            	
            case TYPE_RIGHT_TEXT:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_msg_date"));
            	mMsg = (EmojiconTextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_msg_text"));
            	mSendFailedIv = (ImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_msg_fail_iv"));
            	mSendingPb = (ProgressBar) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_msg_out_pb"));
            	mAvatar = (RoundImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_msg_avatar"));
            	break;
            	
            case TYPE_SINGLE_NEWS:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_news_msg_date"));
            	mNewsPic = (ImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_news_img"));
            	mNewsTitle = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_news_title_inner_text"));
            	mNewsContent = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_news_desc_text"));
            	break;
            	
            case TYPE_NEWS:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_news_msg_date"));
            	mNewsListLayout = (ListLinearLayout) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_news_layout"));
            	break;
            	
            case TYPE_LOCATION_R:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_msg_date"));
            	mMapIv = (ImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "ic_map_img_iv"));
            	mLbsDescTv = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_map_address_text"));
            	mSendFailedIv = (ImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_msg_fail_iv"));
            	mSendingPb = (ProgressBar) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_msg_out_pb"));
            	mAvatar = (RoundImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_msg_avatar"));
            	break;
            ...
            case TYPE_IMG_L:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_date"));
            	mMapIv = (ImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "ic_type_img_iv"));
            	mAvatar = (RoundImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_avatar"));
            	break;
            ...
            case TYPE_VOICE_L:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_date"));
            	mVoiceIv = (ImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_voice_iv"));
            	mVoiceSecondTv = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_voice_tv"));
            	mAvatar = (RoundImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_avatar"));
            	break;
            	
            case TYPE_VOICE_R:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_msg_date"));
            	mVoiceIv = (ImageView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_voice_iv"));
            	mVoiceSecondTv = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_to_voice_tv"));
            ...
            case TYPE_TIPS:
            	mDate = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_from_msg_date"));
            	mMsg = (TextView) itemView.findViewById(UIUtil.getIdByName(mContext, "id", "id_msg_tips"));
            	break;
        ...
        public void startVoicePlaying() {
			...
			mVoiceIv.setBackgroundResource(UIUtil.getIdByName(mContext, "anim", "v5_anim_rightgray_voice"));
			if (msg.getDirection() == V5MessageDefine.MSG_DIR_TO_WORKER) {
				...
				mVoiceIv.setBackgroundResource(UIUtil.getIdByName(mContext, "anim", "v5_anim_leftwhite_voice"));
			}
...
//			}
			...
			if (msg.getDirection() == V5MessageDefine.MSG_DIR_TO_WORKER) {
				mVoiceIv.setBackgroundResource(UIUtil.getIdByName(mContext, "drawable", "v5_chat_animation_right_gray3"));
			} else if (msg.getDirection() == V5MessageDefine.MSG_DIR_FROM_ROBOT ||
					msg.getDirection() == V5MessageDefine.MSG_DIR_TO_CUSTOMER) {
				mVoiceIv.setBackgroundResource(UIUtil.getIdByName(mContext, "drawable", "v5_chat_animation_left_white3"));
			}
		}
...
//			if (v.getId() == UIUtil.getIdByName(mContext, "id", "id_left_voice_layout") ||
//					v.getId() == UIUtil.getIdByName(mContext, "id", "id_right_voice_layout")) {
